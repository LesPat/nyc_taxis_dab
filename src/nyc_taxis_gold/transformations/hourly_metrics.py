"""hourly_metrics: grouped by date_time -> metrics:
...
from pyspark.sql.functions import col, avg, count, sum, max, min, date_trunc
from pyspark import pipelines as dp
from nyc_taxis_dab.taxis import read_silver_table 

@dp.table(name="hourly_metrics")
def hourly_metrics():
    silver = read_silver_table().withColumn("pickup_hour", date_trunc("hour", col("pickup_datetime")))
    return silver.groupBy(col("pickup_hour")).agg(count("*").alias("count_trips"),
